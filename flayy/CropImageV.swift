...
class CropImageV: UIView,  CornerProtocol{
    ...
    @IBOutlet var  cropDelegate: CropImageViewDelegateProt?
    ...
    required init?(coder aDecoder: NSCoder)
    {
        ...
        dragger.addTarget(self as AnyObject, action: #selector(CropImageV.handleDragInView(_:)))
        viewForImage.frame = frame;
        addGestureRecognizer(dragger)
            
        let tapper = UITapGestureRecognizer(target: self as AnyObject,
                                            action: #selector(CropImageV.handleViewTap(_:)));
